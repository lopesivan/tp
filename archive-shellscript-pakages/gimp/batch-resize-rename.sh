...
if [ $__help__ == 1 ]; then
    ...
    echo
        echo "This particular script takes a pattern for filename, desired width and"
        echo "height as inputs and resize all image files that are matched. This script"
        echo "overwrite the existing file."
    echo
    exit 1
fi
scm=batch-resize-rename
