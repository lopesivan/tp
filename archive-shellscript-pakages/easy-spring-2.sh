...
# ----------------------------------------------------------------------------
b.set "Roo.TemplateSourcePath" "/opt/files/java/spring/02"
...
function roo_script() {
      roo project --topLevelPackage $1 --java 7 --projectName $2 --packaging JAR
      create_class_with_roo
...
function copy_src_to_roo_project () {
  cp $1/service/*.java src/main/java/${2//./\/}/service/ 
  cp $1/*.java src/main/java/${2//./\/}/ 
}

function update_template_package () {
   sed "s/com.foo/$1/" -i src/main/java/${1//./\/}/*.java
   sed "s/com.foo/$1/" -i src/main/java/${1//./\/}/service/*.java
}

function create_class_with_roo () {
  roo interface --class ~.service.MessageService
  roo class --class ~.service.MessagePrinter
  roo class --class ~.service.MyBean
  roo class --class ~.service.MyBean2
  roo class --class ~.Application
}
